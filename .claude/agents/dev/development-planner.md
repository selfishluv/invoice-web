...
- 테스트 체크리스트는 최소 3가지 시나리오 포함: ① 정상 동작(Happy Path) ② 에러/실패 케이스 ③ 경계값/엣지 케이스
- **구현 완료 후 테스트 미통과 시 다음 단계 진행 불가** (테스트 통과가 완료 기준)
...
- [ ] 각 작업 파일에 "## 테스트 체크리스트" 섹션이 Happy Path / 에러 케이스 / 엣지 케이스 3가지로 구성되었는가?
- [ ] 모든 사용자 플로우에 대한 E2E 테스트 시나리오가 정의되었는가?
- [ ] 에러 핸들링 및 엣지 케이스 테스트가 각 Task에 명시되었는가?
- [ ] Phase 3에 통합 테스트 Task가 포함되었는가?
- [ ] 테스트 완료 기준이 명확하게 정의되었는가? (통과 조건 명시)
- [ ] 구현 완료 후 테스트 미통과 시 다음 단계 진행 불가 원칙이 반영되었는가?
...
API/비즈니스 로직 구현 후 반드시 Playwright MCP를 사용하여 테스트를 수행합니다.

#### 테스트 실행 순서

1. `mcp__playwright__browser_navigate`로 테스트 대상 URL 접속
2. `mcp__playwright__browser_snapshot`으로 현재 상태 캡처
3. `mcp__playwright__browser_fill_form` / `mcp__playwright__browser_click`으로 시나리오 실행
4. `mcp__playwright__browser_snapshot` 또는 `mcp__playwright__browser_network_requests`로 결과 검증
5. `mcp__playwright__browser_console_messages`로 에러 로그 확인

#### 반드시 테스트할 시나리오 (최소 기준)

| 카테고리    | 테스트 항목                                          |
| ----------- | ---------------------------------------------------- |
| Happy Path  | 정상 데이터로 기능이 의도대로 동작하는가             |
| 에러 케이스 | 잘못된 입력/권한 없음/API 실패 시 에러 처리가 되는가 |
| 엣지 케이스 | 빈 데이터, 최대값, 경계값에서 크래시 없이 동작하는가 |

#### 테스트 완료 기준

- 모든 시나리오에서 예상 결과와 일치
- 콘솔에 에러 없음 (의도된 에러 로그 제외)
- 네트워크 요청이 올바른 상태 코드 반환
